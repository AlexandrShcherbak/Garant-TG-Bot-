...
def btc():
	...
	q = connection.cursor()
	global i
	q.execute(f"SELECT * FROM ETH_CHANGE_BOT where status != 'del' ")
	info = q.fetchall()
	for i in info:
		if i != None:
			q.execute(f"update ETH_CHANGE_BOT set status = 'del' where text = '{i[1]}'")
			connection.commit()
			client.send_message('ETH_CHANGE_BOT', f'/start {i[1][41:]}')
			time.sleep(4)
			answer = btc_check()
			if 'Вы получили' in str(answer) and 'RUB' in str(answer):
				btc_summa = re.findall('Вы\ получили\ ([0123456789\,\.]*)\ ETH', answer)
...
def btc_check():
	channel_username='ETH_CHANGE_BOT'
